...
public class ClientView extends CustomComponent implements View {
	...
	private Label label_title, label_lastname, label_firstname,
			label_birthdate, label_street, label_zip, label_city, label_country, label_status, label_mobile,
	...
	private TextField field_lastname, field_firstname,
			field_mobile, field_email, field_street, field_zip, field_city, field_country, field_showstatus;
	Label field_doctor;
	Label field_emergency;
	private DateField field_birthdate;
	
	private boolean isHidden = true;
	private PropertysetItem client = new PropertysetItem();
	...
	private AbsoluteLayout buildMainLayout() {
		...
		//field street: displays street of client address
		field_street = new TextField();
		field_street.setImmediate(false);
...
//	}
	private void loadClientData(Client c){
		FieldGroup binder = new FieldGroup(client);
		client.addItemProperty("lastname", new ObjectProperty<String>(c.getLastName()));
		binder.bind(field_lastname, "lastname");
		client.addItemProperty("firstname", new ObjectProperty<String>(c.getFirstName()));
		binder.bind(field_firstname, "firstname");
		client.addItemProperty("birthday", new ObjectProperty<Date>(c.getBirthday().getTime()));
		binder.bind(field_birthdate, "birthday");
		client.addItemProperty("mobile", new ObjectProperty<String>(c.getMobileNr()));
		binder.bind(field_mobile, "mobile");
		client.addItemProperty("email", new ObjectProperty<String>(c.getEmail()));
		binder.bind(field_email, "email");
		client.addItemProperty("street", new ObjectProperty<String>(c.getAddress().getStreet()));
		binder.bind(field_street, "street");
		client.addItemProperty("zip", new ObjectProperty<String>(c.getAddress().getZip()));
		binder.bind(field_zip, "zip");
		client.addItemProperty("city", new ObjectProperty<String>(c.getAddress().getCity()));
		binder.bind(field_city, "city");
		client.addItemProperty("country", new ObjectProperty<String>(c.getAddress().getCountry()));
		binder.bind(field_country, "country");
//		client.addItemProperty("emergencyContact", new ObjectProperty<String>(c.getEmergencyContact().getFirstName() + " " + c.getEmergencyContact().getLastName()));
//		client.addItemProperty("emergencyContact", new ObjectProperty<String>("Philip J. Fry"));
//		binder.bind(field_emergency, "emergencyContact");
		field_emergency.setValue(c.getEmergencyContact().getFirstName() + " " + c.getEmergencyContact().getLastName());
//		client.addItemProperty("doc", new ObjectProperty<String>(c.getDoc().getFirstName() + " " + c.getDoc().getLastName()));
//		client.addItemProperty("doc", new ObjectProperty<String>("Doktor Zoidberg"));
//		binder.bind(field_doctor, "doc");
		field_doctor.setValue(c.getDoc().getFirstName() + " " + c.getDoc().getLastName());
